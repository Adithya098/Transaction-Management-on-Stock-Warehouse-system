...
from test_new_order_transaction import new_order_transaction
# from order_status import order_status_transaction
from test_payment_transaction import payment_transaction
from test_delivery_transaction import delivery_transaction
import time
from stock import check_stock_threshold
from top_balance_customers_test import top_balance_customer
from popular_item_test import popular_items_transaction
from related_customers_test import related_customers_transaction
from test_order_status import order_status_transaction
    ...
    for line in fi:
        ...
        try:
            if t_type == 'N':
                ...
                new_order_transaction(session, w_id, d_id, c_id, n_lines, ol_i_ids, ol_supply_w_ids, ol_quantities)
                ...
                payment_transaction(session, c_w_id, c_d_id, c_id, payment)
                P_trans_time = P_trans_time + ((time.time()-transaction_start_time)*1000)
                P_trans_count = P_trans_count + 1

            elif t_type == 'D':
                w_d_id, carrier_id = vals[1:]
                w_d_id = int(w_d_id)
                carrier_id = int(carrier_id)
                delivery_transaction(session, w_d_id, carrier_id)
                ...
                C_D_ID = int(C_D_ID)
                C_ID = int(C_ID)
                order_status_transaction(session, C_W_ID, C_D_ID, C_ID)
                ...
                T = int(T)
                L = int(L)
                #print(f"I am here and params are {W_ID}, {D_ID}, {T}, {L}")
                check_stock_threshold(session, W_ID, D_ID, T, L)
                ...
                D_ID = int(D_ID)
                L = int(L)
                popular_items_transaction(session, W_ID, D_ID, L)
                ...
                D_ID = int(D_ID)
                C_ID = int(C_ID)
                #print(f"W_ID is {W_ID} and D_ID is {D_ID} and C_ID {C_ID}")
                related_customers_transaction(session, W_ID,D_ID,C_ID)
